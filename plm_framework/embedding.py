...
class ProteinEmbedder:
    ...
    def __init__(
        ...
        if not isinstance(device, str) and device is not None:
            logger.warning(
                f"device is not a string: {type(device)}. Using default device."
            )
            device = None
        ...
        if quantize:
            ...
            self.model = AutoModel.from_pretrained(model_name, cache_dir=cache_dir).to(
                self.device
            )
    ...
    def embed_variants(self, variants: List[Variant]) -> Tuple[List[int], np.ndarray]:
        """
        ...
        if not variants:
            logger.warning("Empty variants list provided to embed_variants")
            return [], np.array([])
